...
description: >-
  ...
  what-runs-next question. The runnable script lives in this shared skill's own
  scripts/ directory, not at repo-root scripts/; see Commands for invocation.
---
...
The runnable script is bundled with this shared workflow, in `scripts/` next to
this file -- not at the repo root. The repo root holds only the `roadmap/`
store. Run from the repo root (so the default `--root roadmap` finds the store):

```
python3 .agents/skills/roadmap-items/scripts/roadmap.py <command> [options]
```

If your CWD is not the repo root, add `--root /abs/path/to/roadmap`. The
discovery adapters for Claude and Codex both delegate here; neither carries a
second copy of the workflow or script. The examples below abbreviate the chosen
invocation as `roadmap.py`.
