...
emit("Fails in the model (model middle is rigid).  Its LOCAL cost is")
emit("dimension-1: O(1) rho-algebra factors (M7), so it passes every")
emit("budget -- the block is ABSENCE from the corpus, not cost.")
emit("x        local-cost expo   carrier present?   CG effect")
for e in SCALES:
    lnx, llx, L, k = regime(e)
    emit("1e%-5d %14.3f   NO (F20.4/F20.5)   weakens strength only"
         % (e, 0.0))
emit("  -> expo 0 (affordable at every scale): the dimension-1 middle")
emit("     law carries O(1) constants.  CG relaxes the DEMANDED strength")
emit("     from full equidistribution (o(1) error) to constant-order")
emit("     non-concentration on sparse scales per s -- a strictly weaker")
emit("     target -- but does NOT manufacture the missing carrier.")
emit("     VERDICT: right shape, absent from corpus; the weaker CG")
emit("     variant is UNDECIDABLE at corpus grain.")
emit("")
...
emit("  -> a constant-order middle law leaves full reserve; the reserve")
emit("     is never the binding constraint for S1.  S1's block is carrier")
emit("     ABSENCE (T9.S1), not budget.  (An (abs)-route S1 with a per-")
emit("     word sieve constant (ln x)^2-class is dead, as in the anchor.)")
emit("")
