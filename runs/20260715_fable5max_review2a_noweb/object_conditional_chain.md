# Conditional irrationality of S = sum p_n/2^n
...
"tail differs from an integer" clause).
...
- No logical independence of B from A is claimed. What is true: the naive
  ...
  (exponent bookkeeping). Whether some other derivation of B
...
- No claim that the Lean skeleton compiles or that the analytic layer is
  ...
  formalization is separate work.
...
of the surveyed architectures is one kernel in ordered formulations:
...
plausibly provable, but this document does not need it.
8.2 Lean interface. Target theorem on 2S with thresholds
